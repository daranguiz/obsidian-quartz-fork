...
- [X] T001 Review Explorer component current implementation in quartz/components/Explorer.tsx
- [X] T002 Review Explorer inline script current implementation in quartz/components/scripts/explorer.inline.ts
- [X] T003 Review current Explorer configuration in quartz.layout.ts
...
- [X] T004 Add autoExpandFolders option to Options interface in quartz/components/Explorer.tsx
- [X] T005 Pass autoExpandFolders via data attribute in Explorer.tsx render method
...
- [X] T006 [US1] Read autoExpandFolders from data attribute in quartz/components/scripts/explorer.inline.ts (after line 203)
- [X] T007 [US1] Implement state injection logic for auto-expand folders in explorer.inline.ts (after loading saved state, before folder creation)
- [X] T008 [US1] Configure autoExpandFolders for lower tiers in quartz.layout.ts (left sidebar Explorer configuration)
- [X] T009 [US1] Configure autoExpandFolders for lower tiers in quartz.layout.ts (right sidebar Explorer configuration if present)
...
- [X] T010 [US2] Verify autoExpandFolders is empty array for Full tier in quartz.layout.ts configuration
- [X] T011 [US2] Test Full tier build locally with `npx quartz build --publish-mode full --serve`
- [X] T012 [US2] Verify no auto-expansion occurs on Full tier (all folders collapsed by default)
...
- [X] T013 [US3] Document configuration format in quickstart.md (already completed during planning)
- [X] T014 [US3] Verify state injection logic only processes folders in autoExpandFolders array (no implicit expansion)
- [X] T015 [US3] Test adding new folder to vault and verify it stays collapsed on lower tiers
...
- [X] T016 [P] Test Full tier locally - verify all folders collapsed on page load
- [X] T017 [P] Test Trusted tier locally - verify "Tea Resources" and "紙片 (Shihen)" auto-expanded on page load
- [X] T018 [P] Test Shachu tier locally - verify "Tea Resources" and "紙片 (Shihen)" auto-expanded on page load
- [X] T019 [P] Test Public tier locally - verify "Tea Resources" and "紙片 (Shihen)" auto-expanded on page load
- [X] T020 Test state persistence - manually collapse auto-expanded folder, refresh, verify it stays collapsed (saved state wins)
- [X] T021 Test state reset - clear localStorage, refresh, verify auto-expansion reapplies
- [X] T022 Test edge case - nested folder without parent (verify parent must be in config)
- [X] T023 Test edge case - folder doesn't exist (verify graceful handling, no errors)
- [X] T024 Verify click count reduction - Shihen files accessible in 1 click on lower tiers vs 3 clicks on Full tier
- [X] T025 Update ARCHITECTURE.md with smart navigation defaults section in docs-custom/ARCHITECTURE.md
- [X] T026 Verify no performance degradation - check page load times across tiers
