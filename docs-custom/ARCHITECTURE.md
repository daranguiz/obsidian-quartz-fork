...
##### Inline Tag Display (CUSTOM)

**Feature**: 005-inline-tag-display
**Location**:
- Bug fix: `quartz/plugins/emitters/contentPage.tsx` (lines 56-65)
- Layout change: `quartz.layout.ts` (TagList removed from beforeBody)
- CSS (unchanged): `quartz/styles/base.scss` (lines 123-127)

**What it does**: Tags appear inline within content at their exact markdown position, styled as regular links with the # symbol visible, rather than in a separate section at the top of pages.

**Behavior**:
- **Inline tags** (`#tag` or `#category/subtag` in markdown): Display at their exact position in content as clickable links
- **Hash symbol**: Always visible before tag names (e.g., "#tea" not just "tea")
- **Top-of-page tags section**: Removed entirely (TagList component no longer included in layout)
- **Tag navigation**: Clicking inline tags navigates to tag listing pages (`/tags/{tag}`)
- **Tag indexing**: Frontmatter tags remain indexed for tag page generation but are not displayed separately
- **Link styling**: Tags styled identically to regular internal/external links (blue color, hover effects)

**Implementation Details**:
1. **Bug Fix (contentPage.tsx)**: Dead link detection now skips elements with `tag-link` class. Previously, tag links were incorrectly converted from `<a>` to `<span>` elements because tag page URLs aren't in `allSlugs` (they're generated dynamically). The fix checks for `tag-link` class and returns early to preserve proper `<a>` elements.

2. **Layout Change (quartz.layout.ts)**: Removed `Component.TagList()` from the `beforeBody` array in `defaultContentPageLayout`. This eliminates the pill-styled tags section that previously appeared at the top of pages.

3. **CSS (Unchanged)**: The existing CSS rule `a.tag-link::before { content: "#"; }` was already correct and now works properly with the bug fix.

**Processing Flow**:
1. `ofm.ts` transformer parses inline `#tag` syntax in markdown
2. Converts to MDAST link nodes with `internal tag-link` classes
3. Inline tags automatically added to `frontmatter.tags` array
4. Tag links point to `/tags/{tag}` URLs (generated by TagPage emitter)
5. `contentPage.tsx` emitter now preserves `<a>` elements for tags (previously broken)
6. Rendered HTML shows tags inline with # symbol and clickable navigation

**Status**: Custom bug fix + layout change (005-inline-tag-display)
